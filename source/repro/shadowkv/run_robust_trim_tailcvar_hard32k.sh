#!/usr/bin/env bash
# Robust-placement / tail-sensitive-allocation end-to-end screen.
#
# Grid: Qwen3 32K, four hard RULER tasks, B in {512,2048}, mean centers in
# {1.25,1.5}, and either one-shot selection or exact-token reranking from a
# 2B candidate set.  Work can be partitioned across machines using modular
# slots exactly as in run_center_block_matrix.sh.
set -euo pipefail
...
ROOT=${ROBUST_TAIL_ROOT:-/storage/baonn/robust_trim_tailcvar_hard32k_20260908}
STATE=$ROOT/.state
NUM_SAMPLES=${NUM_SAMPLES:-30}
GPUS=${SHADOWKV_POOL_GPUS:-0,1,2,3,4,5,6,7}
TOTAL_SLOTS=${ROBUST_TOTAL_SLOTS:-1}
SLOT_START=${ROBUST_SLOT_START:-0}
SLOT_COUNT=${ROBUST_SLOT_COUNT:-1}
  ...
  while true; do
    ...
    if (
      ...
      "$HERE/run_cell.sh" qwen3 32768 "$task" \
        adaptive_centroid_lse_streaming_prefix4 "$budget" 160 8 16 0 \
        "$NUM_SAMPLES" "$gpu" max
    ) >"$log" 2>&1; then
...
wait "${pids[@]}"
echo "robust-tail campaign complete"
