#!/usr/bin/env bash
# Extend the paired query-mean study beyond rbar={1.25,1.5}.  Those two points
# live in run_robust_trim_tailcvar_querymean_hard32k.sh; this complementary
# sweep evaluates rbar={1,2,3,4,6,8} without duplicating completed cells.
set -euo pipefail
...
ROOT=${CENTER_SWEEP_ROOT:-/storage/baonn/querymean_center_budget_sweep_hard32k_20260908}
STATE=$ROOT/.state
NUM_SAMPLES=${NUM_SAMPLES:-30}
GPUS=${SHADOWKV_POOL_GPUS:-0,1,2,3,4,5,6,7}
TOTAL_SLOTS=${CENTER_SWEEP_TOTAL_SLOTS:-1}
SLOT_START=${CENTER_SWEEP_SLOT_START:-0}
SLOT_COUNT=${CENTER_SWEEP_SLOT_COUNT:-1}
...
echo "cells=96 samples=$NUM_SAMPLES slots=${SLOT_START}..$((SLOT_START+SLOT_COUNT-1))/$TOTAL_SLOTS gpus=$GPUS root=$ROOT"
wait "${pids[@]}"
echo "query-mean center-budget sweep complete"
