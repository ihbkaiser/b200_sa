...
from .compat import rmsnorm

try:
    # Only reachable behind --minference; kept optional so the release runs
    # without the (torch-2.3-pinned) MInference package installed.
    from minference import vertical_slash_sparse_attention, block_sparse_attention, streaming_forward
    HAS_MINFERENCE = True
except Exception:
    vertical_slash_sparse_attention = block_sparse_attention = streaming_forward = None
...
def layer_norm(
    ...
    return rmsnorm(hidden_states, w, eps)
