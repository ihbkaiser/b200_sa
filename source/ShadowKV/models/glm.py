...
try:
    import vllm
except ImportError:  # GLM needs vllm's partial-rotary and silu_and_mul kernels.
    # vllm pins torch 2.3.1, which cannot load Qwen3, so it is not installed in
    # this environment. Importing models/ must still work; GLM fails loudly at
    # construction rather than silently taking an unvalidated fallback.
    vllm = None
...
class GLM(LLM):
    def __init__(self, 
        ...
        if vllm is None:
            raise ImportError(
                "GLM requires vllm (partial-rotary and silu_and_mul kernels); it is not "
                "installed in this environment. See models/compat.py for why.")
