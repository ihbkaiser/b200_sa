...
from .compat import rotary_embedding_neox_, get_inv_freq, head_dim_of, silu_and_mul
...
class Llama(LLM):
    def __init__(self, 
        ...
        self.init_kv_cache(sparse_budget, rank, chunk_size, self.config,
                           ...
                           m51_pq_mode=m51_pq_mode, m51_group_select=m51_group_select,
    ...
    def apply_rotary_pos_emb(self, q: torch.Tensor, k: torch.Tensor, position_ids: torch.Tensor) -> torch.Tensor:
        rotary_embedding_neox_(position_ids, q, k, self.head_dim, self.cos_sin_cache)
        bsz = q.shape[0]
    ...
    def init_parameters(self):
        ...
        try:
            ...
            cos_cache, sin_cache = self._set_cos_sin_cache(get_inv_freq(hf_model).to(self.device))
        half_dim = self.head_dim // 2
        self.cos_sin_cache = torch.cat((cos_cache[:, :half_dim], sin_cache[:, :half_dim]), dim=-1)
